(function ($) {
    ...
    new WOW().init();
    let defaultValue = [0, 5000000];
    let price_max_calc = 5000000;

    function GetdefaultValue() {
        var pMin = $("#price-min").val();
        var pMax = $("#price-max").val();
        if (pMin !== undefined && pMin !== "") {
            defaultValue[0] = parseInt(pMin);
            return defaultValue;
        }
        if (pMax !== undefined && pMax !== "") {
            defaultValue[1] = parseInt(pMax);
            return defaultValue;
        }
        return defaultValue;
    }
    function Getprice_max_calc() {
        var maxPrice = $("#price_max_calc_dynamic").val();

        if (maxPrice !== undefined && maxPrice !== "") {
            return parseInt(maxPrice);
        }
        else {
            return price_max_calc;
        }
